...
public class LQSSieveParams {
	...
	public LQSSieveParams(BigInteger kN, int[] primeBase, int primeBaseSize, long sieveArrayArea, int minLnPSumStyle, int specialqSize, double maxQRest, int wantedMinLogPSum) {
		...
		// This bound is computed as average ln(Q/special_q) - ln(maxQRest) values.
		...
		case 1: {
			...
			// For bigq this requires a maxQRestExponent ~ 0.1575 // TODO smallq ?
			minLnPSum = Math.log(sieveArrayArea) + Math.log(kN.doubleValue())/2 - Math.log(specialqSize) - Math.log(maxQRest);
			break;
		}
		case 2: {
			// Subtract nothing; just an intermediate choice between the two extremes, case 1 and case 3.
			// For bigq this requires a maxQRestExponent ~ 0.22
			minLnPSum = Math.log(sieveArrayArea) + Math.log(kN.doubleValue())/2 - Math.log(maxQRest);
			break;
		}
		case 3: {
			// The theoretically correct solution: Add ln(special_q) ~ ln(pMax)
			// For bigq this requires a maxQRestExponent ~ 0.28
			minLnPSum = Math.log(sieveArrayArea) + Math.log(kN.doubleValue())/2 + Math.log(specialqSize) - Math.log(maxQRest);
			break;
		}
